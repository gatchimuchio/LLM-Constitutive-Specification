...
def 監査() -> list[str]:
    ...
    if データ.get("言語模型性成立条件") != 言語模型性成立条件:
        問題.append("言語模型性成立条件がv3正本の五区別と一致しない")
    if データ.get("構成再現条件") != 構成再現条件:
        問題.append("構成再現条件がv3正本の七区別と一致しない")
    if データ.get("規模面") != ["状態域規模", "関係域規模", "共有適用規模"]:
        問題.append("規模三面がv3正本と一致しない")
